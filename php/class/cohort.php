...
class Cohort
{
   ...
   /**
    * Selects most recent Cohorts regardless of role
    *
    * @param resource $mysqli pointer to mySQL connection, by reference
    * @return null
    * @throws mysqli_sql_exception when mySQL related errors occur
    *
    **/
   public static function getRecentCohortByProfileId(&$mysqli, $profileId) {
      // handle degenerate cases
      if(gettype($mysqli) !== "object" || get_class($mysqli) !== "mysqli") {
         throw(new mysqli_sql_exception("input is not a mysqli object"));
      }
      ...
      if($results->num_rows > 0) {
         ...
         // step through results array and convert to Topic objects
         foreach ($results as $index => $row) {
            $results = new cohort($row["cohortId"], $row["startDate"], $row["endDate"], $row["location"], $row["description"]);
         }
	...
	public static function getCohortObjects(&$mysqli) {
		// handle degenerate cases
		if(gettype($mysqli) !== "object" || get_class($mysqli) !== "mysqli") {
			throw(new mysqli_sql_exception("input is not a mysqli object"));
		}
